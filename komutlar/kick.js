...
exports.run = (client, message, args) => {
  ...
  let rooms = message.guild.channels.cache.find(x => x.id  === denkanal);
    ...
    .setDescription('📋 ' + message.member.user.username + '** .kick <kullanıcıetiketi> [Sebep]** Şeklinde Yazmanız Gerekmektedir!')
    .setColor(Math.floor(Math.random()*16777215))
    ...
    .setTitle('Bilgi Sistemi')
    .setDescription("**" + message.member.user.username + '** Herhangi Bir Kullanıcıyı Sunucudan Atmak İstiyorsan **Kick Hammer | 𓇼** Rolüne Sahip Olmalısın!')
    .setColor(Math.floor(Math.random()*16777215))

    let banrol = message.guild.roles.cache.find(x=> x.name === "Kick Hammer | 𓇼");
    if(!banrol) return message.reply("`Kick Hammer | 𓇼` Rolü Bulunamadı Bu Rolü Oluşturman gerekiyor!")

    if (!message.member.roles.cache.find(x => x.name === "Kick Hammer | 𓇼")) return message.channel.send(hata2);
    ...
    .setDescription('**<@' + user + '>**,** ' + message.member.user.username + '** Tarafından ' + sebep +' Sebebiyle Sunucudan Atıldı!')
    .setColor(Math.floor(Math.random()*16777215))
